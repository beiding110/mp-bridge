...
const color = ['#3b68ff', '#2ec75a', '#ff8f00', '#ff5454', '#3b6899', '#2ec880', '#ff8f99', '#ff6666'];
...
const cfu = {
	...
	"formatter": {
		...
		"barPercentFormat": function(val, index, series, opts) {
			if (!series) {
				return val;
			}
			...
			sum = data.reduce((sum, item) => {
					sum += item;

					return sum;
				}, 0),
				percent = ((val / sum) * 100).toFixed(2);

			return `${val}(${percent}%)`;
		},
		pieIndustryNumPercent(item, index, series, opts) {
			if (index !== undefined) {
				var {
					data,
					_proportion_
				} = item,
				percent = (_proportion_ * 100).toFixed(2),
					name = item.name;

				const SHOW_NAME_LENGTH = 4;

				if (name.length > SHOW_NAME_LENGTH) {
					name = name.slice(0, SHOW_NAME_LENGTH) + '...';
				}
